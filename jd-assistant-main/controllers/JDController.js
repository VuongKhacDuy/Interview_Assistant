    ...
    try {
        if (!process.env.GEN_API_KEY) {
            res.render('jd', {
                title: 'JD Assistant',
                showApiKeyForm: true,
                message: 'Bạn chưa có GPT KEY, hãy thực hiện cách sau và nhập KEY vào đây để tiếp tục. API key lấy tại đây https://aistudio.google.com/apikey'
            });
        } else {
            res.render('jd', {
                title: 'JD Assistant',
 ...
 * - Trả về nhận xét chi tiết kèm gợi yếu cải thiện (nếu cần).
 */
exports.evaluateAnswer = async (req, res) => {
    try {
        ...
        const prompt = `Bạn là một chuyên gia nhân sự giàu kinh nghiệm và rất khó tính, được giao nhiệm vụ đánh giá chi tiết câu trả lời của ứng viên cho câu hỏi phỏng vấn duy nhất dưới đây, được tạo dựa trên mô tả công việc (JD).
            ...
            5. Nếu điểm dưới 6, hãy đưa ra gợi yếu cụ thể để cải thiện; nếu điểm từ 6 trở lên, chỉ cần nhận xét tóm tắt.
...
exports.submitApiKey = async (req, res) => {
    try {
        ...
        global.GEN_API_KEY = apiKey;

        // Redirect to the main page with success message
        res.redirect('/jd?success=true');
    } catch (error) {
        console.error('Error submitting API key:', error);
        res.status(500).json({ error: 'Failed to submit API key' });
    }
};